...
class Fantasy_Service:
	...
	def get_rankings(self):
		...
		for team in self.teams:
			dict_of_ranks[team[0]] = int(values_rank[i][0])
			i += 1
		...
		for score in self.scores:
			dif = dict_of_ranks[score.team_name] - dict_of_ranks[score.vs_team_name]
			if score.diff > 0 and dif >= 3:
				high_rank = dict_of_ranks[score.team_name]
				low_rank = dict_of_ranks[score.vs_team_name]
				lowest_winner = score
