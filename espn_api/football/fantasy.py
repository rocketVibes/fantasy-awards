...
class FantasyService:
    ...
    def process_matchup(self, lineup, team_name, score, opp_score, owner_name, vs_team_name, vs_owner):
        ...
        for player in lineup:
            ...
            if player.points >= 50:
                self.award(team_name, f'50 BURGER ({player.name}, {player.points})', player.lineupSlot + '_HIGH',
                           player.points * 1000)
            ...
            elif player.lineupSlot == 'TE':
                self.tes.append(new_player)
            elif player.lineupSlot == 'K':
            ...
            elif player.lineupSlot == 'WR':
                self.wrs.append(new_player)
            elif player.lineupSlot == 'WR/TE':
                self.wrs.append(new_player)
            elif player.lineupSlot == 'D/ST':
    ...
    def compute_start_sit(self, roster, pos, lineup_slot, team_name, diff):
        ...
        if benched_player is not None and diff < 0 and benched_player.points > starter.points:
            ...
            if benched_player.points >= abs(diff) + starter.points:
                return (f'BLUNDER - Started {benched_player.name} ({benched_player.points}) over {starter.name} '
                        f'({starter.points}) (lost by {round(abs(diff), 2)})',
